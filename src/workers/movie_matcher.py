...
class MovieMatcher(Worker):
    ...
    async def execute(
        ...
        if not movie_details:
            if self.logger:
                self.logger.error(f"Could not fetch movie details for TMDB ID: {tmdb_id}")
            return []
        ...
        if entity_id:
            if self.logger:
                self.logger.info(f"Successfully matched and inserted movie: {movie_dto.title} (TMDB ID: {tmdb_id})")
        else:
            if self.logger:
                self.logger.error(f"Failed to create entity for movie: {movie_dto.title} (TMDB ID: {tmdb_id})")
