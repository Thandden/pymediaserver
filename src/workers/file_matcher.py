...
from src.common.dto import MatchedData, FileMatcherParams, ChildJobRequest, MetadataMatcherParams
...
class FileMatcher(Worker):
    ...
    def _clean_title(self, title: str) -> str:
        ...
        if hasattr(self, '_last_matched_year_pattern') and hasattr(self, '_last_matched_year'):
    ...
    def _match_file_path(self, file_path: str, media_type: MediaType) -> MatchedData:
        ...
        if media_type == MediaType.TV:
            ...
            if season_number is None and parent_dir:
                ...
                season_match = re.search(r'[Ss]eason[. ]?(\d{1,2})', parent_dir, re.IGNORECASE)
                if not season_match:
            ...
            if grandparent_dir and grandparent_dir.lower() not in {'tv', 'shows', 'series', 'television'}:
                title = grandparent_dir
            # Then try parent directory
            elif parent_dir and not re.search(r'^(?:[Ss]eason[. ]?\d+|[Ss]\d+)$', parent_dir, re.IGNORECASE):
                title = parent_dir
            ...
            # Next try the directory containing "movies" or "films"
                ...
                movie_dir_index = next((i for i, x in enumerate(parts) 
                                     if x.lower() in ['movies', 'films']), -1)
