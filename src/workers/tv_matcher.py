...
class TVMatcher(Worker):
    ...
    async def execute(
        ...
        if not show_details:
            if self.logger:
                self.logger.error(f"Could not fetch TV show details for TMDB ID: {tmdb_id}")
            return []
        ...
        for season_data in seasons_data:
            ...
            detailed_season = await self._fetch_season_details(tmdb_id, current_season_number)
            if not detailed_season:
                if self.logger:
                    self.logger.warning(f"Could not fetch season {current_season_number} details for show ID: {tmdb_id}")
                continue
            ...
            if not tv_season_id:
                if self.logger:
                    self.logger.error(f"Failed to insert season {current_season_number} for show ID: {tmdb_id}")
                continue
        ...
        if not target_season_details:
            if self.logger:
                self.logger.error(f"Could not fetch target season {season_number} details for show ID: {tmdb_id}")
            return []
        
        # Find the season ID for our target season
        season_stmt = select(TVSeason).where(
            (TVSeason.show_id == tv_show_id) & 
        ...
        async for session in self.db_session.get_session():
            try:
                ...
                if not target_season:
                    if self.logger:
                        self.logger.error(f"Target season {season_number} not found in database")
                    return []
                ...
                if not target_episode:
                    if self.logger:
                        self.logger.error(f"Target episode {episode_number} not found in database")
                    return []
                
                # Fetch detailed episode information for matched_data
                episode_details = await self._fetch_episode_details(tmdb_id, season_number, episode_number)
    ...
    async def _fetch_season_details(self, tmdb_id: int, season_number: int) -> Dict[str, Any]:
        """
    ...
    def _create_tv_season_dto(self, season_data: Dict[str, Any], show_id: UUID) -> TVSeasonDTO:
        """
    ...
    def _create_tv_episode_dto(self, episode_data: Dict[str, Any], season_id: UUID) -> TVEpisodeDTO:
        """
    ...
    async def _insert_entity(self, entity_dto: EntityDTO) -> Optional[str]:
        ...
        async for session in self.db_session.get_session():
            try:
                ...
                entity_data = {
                    k: v for k, v in entity_dto.model_dump().items() 
