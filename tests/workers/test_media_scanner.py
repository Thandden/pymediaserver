...
from src.common.dto import ChildJobRequest, FileDTO, FileMatcherParams, JobType, MediaScannerParams
...
class MockAsyncSession(AsyncSession):
    ...
    async def execute(self, query: Any) -> Result[Any]:
        ...
        mock_scalars.all.return_value = self.execute_result if self.execute_result is not None else []
...
@pytest.fixture
def media_scanner(mock_db_session: MockAsyncDatabaseSession, mock_logger: Logger) -> MediaScanner:
...
async def test_execute_with_invalid_parameters(media_scanner: MediaScanner) -> None:
    ...
    with pytest.raises(ValueError, match="Parameters must be of type MediaScannerParams"):
...
async def test_execute_with_no_files(media_scanner: MediaScanner) -> None:
    ...
    with patch.object(media_scanner, "_scan_directory", return_value=[]):
        result = await media_scanner.execute(
            MediaScannerParams(dir_path="/fake/path", file_extensions=[".mp3"], media_type=MediaType.MUSIC)
        )
...
async def test_execute_with_new_files(media_scanner: MediaScanner) -> None:
    ...
    with (
        patch.object(media_scanner, "_scan_directory", return_value=["/fake/path/file1.mp3", "/fake/path/file2.mp3"]),
        ...
        for job_request in result:
            ...
            assert job_request.params.path in ["/fake/path/file1.mp3", "/fake/path/file2.mp3"]
...
@pytest.mark.asyncio
async def test_get_all_files_empty(media_scanner: MediaScanner, mock_db_session: MockAsyncDatabaseSession) -> None:
...
@pytest.mark.asyncio
async def test_get_all_files_with_data(media_scanner: MediaScanner, mock_db_session: MockAsyncDatabaseSession) -> None:
...
@pytest.mark.asyncio
async def test_update_db(media_scanner: MediaScanner, mock_db_session: MockAsyncDatabaseSession) -> None:
...
async def test_calculate_md5_success(media_scanner: MediaScanner) -> None:
    ...
    with (
        ...
        patch("aiofiles.open", return_value=AsyncMock(__aenter__=AsyncMock(return_value=mock_file)))
    ):
        # We're intentionally accessing protected method for testing
...
async def test_scan_directory_with_matching_files(media_scanner: MediaScanner) -> None:
    ...
    with (
        ...
        patch("os.path.splitext", side_effect=lambda f: (cast(str, f.split(".")[0]), "." + cast(str, f.split(".")[1]))),
...
@pytest.mark.asyncio
async def test_scan_directory_with_no_extensions_filter(media_scanner: MediaScanner) -> None:
