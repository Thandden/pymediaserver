...
from src.common.dto import FFProbeParams, MediaTechnicalInfoDTO, VideoTrackDTO, AudioTrackDTO
...
async def test_execute_success(
    ffprobe: FFProbe, 
    sample_ffprobe_output: Dict[str, Any],
    ...
    with patch.object(ffprobe, "_run_ffprobe", AsyncMock(return_value=sample_ffprobe_output)):
...
async def test_run_ffprobe_exception() -> None:
    ...
    with patch("asyncio.create_subprocess_exec", AsyncMock(side_effect=Exception("Test error"))):
        result = await ffprobe._run_ffprobe("/path/to/file.mp4")
...
def test_extract_technical_info(
    ...
    tech_info = ffprobe._extract_technical_info(sample_ffprobe_output, file_path, file_id)
...
def test_extract_video_track_hdr10(
    ffprobe: FFProbe,
    sample_hdr_ffprobe_output: Dict[str, Any]
) -> None:
...
def test_extract_video_track_dolby_vision(
    ffprobe: FFProbe,
    sample_dolby_vision_ffprobe_output: Dict[str, Any]
) -> None:
    ...
    with patch.object(ffprobe, "_extract_video_track", wraps=ffprobe._extract_video_track) as mock_extract:
        video_track = ffprobe._extract_video_track(stream, 0, tech_info_id)
...
def test_extract_audio_track(ffprobe: FFProbe, sample_ffprobe_output: Dict[str, Any]) -> None:
...
@pytest.mark.asyncio
async def test_get_file_id_from_path(ffprobe: FFProbe, db_session: MockDBSession) -> None:
    """Test get_file_id_from_path method."""
    file_path = "/path/to/file.mp4"
    expected_id = uuid.uuid4()
    
    # Patch the get_file_id_from_path method to avoid DB interaction
    with patch.object(ffprobe, "get_file_id_from_path", AsyncMock(return_value=expected_id)) as mock_get_id:
        result = await mock_get_id(file_path)
        assert result == expected_id


@pytest.mark.asyncio
async def test_get_file_id_not_found(ffprobe: FFProbe, db_session: MockDBSession) -> None:
    """Test get_file_id_from_path when file doesn't exist."""
    file_path = "/path/to/nonexistent.mp4"
    
    # Patch the get_file_id_from_path method to avoid DB interaction
    with patch.object(ffprobe, "get_file_id_from_path", AsyncMock(return_value=None)) as mock_get_id:
        result = await mock_get_id(file_path)
        assert result is None 