...
@pytest.fixture
def metadata_matcher(mock_db_session: AsyncDatabaseSession, mock_logger: Logger) -> MetadataMatcher:
...
async def test_execute_invalid_parameters(metadata_matcher: MetadataMatcher) -> None:
    ...
    with pytest.raises(ValueError, match="Parameters must be of type MetadataMatcherParams"):
...
@pytest.mark.asyncio
async def test_execute_unsupported_media_type(metadata_matcher: MetadataMatcher) -> None:
...
@pytest.mark.asyncio
async def test_search_movie(metadata_matcher: MetadataMatcher, mock_http_client: AsyncMock) -> None:
...
@pytest.mark.asyncio
async def test_search_movie_no_year(metadata_matcher: MetadataMatcher, mock_http_client: AsyncMock) -> None:
...
@pytest.mark.asyncio
async def test_search_tv(metadata_matcher: MetadataMatcher, mock_http_client: AsyncMock) -> None:
    """Test the _search_tv method."""
    metadata_matcher.http_client = mock_http_client
    mock_response = {
        "results": [
            {"id": 67890, "name": "Test TV Show"}
        ]
    }
    mock_http_client.fetch_json.return_value = mock_response
...
async def test_insert_entity(metadata_matcher: MetadataMatcher) -> None:
    ...
    session = metadata_matcher.db_session.get_session.return_value.__aiter__.return_value[0]
