...
@pytest.fixture
def movie_matcher(mock_db_session: tuple[AsyncMock, AsyncMock], mock_logger: MagicMock) -> MovieMatcher:
...
@pytest.fixture
def movie_matcher_with_test_access(mock_db_session: tuple[AsyncMock, AsyncMock], mock_logger: MagicMock) -> MovieMatcher:
...
async def test_execute_invalid_parameters(movie_matcher: MovieMatcher) -> None:
    ...
    with pytest.raises(ValueError, match="Parameters must be of type MovieMatcherParams"):
...
async def test_find_pending_entity_not_found(
    movie_matcher: MovieMatcher,
    mock_db_session: tuple[AsyncMock, AsyncMock]
) -> None:
