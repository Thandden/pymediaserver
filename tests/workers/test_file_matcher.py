...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            MatchedData(
                title="The Shawshank Redemption", year=1994, media_type=MediaType.MOVIE
            ),
        ),
        (
        ...
        (
            ...
            MatchedData(
                title="Spider Man Far From Home", year=2019, media_type=MediaType.MOVIE
            ),
        ),
    ],
)
def test_movie_matching(
    file_matcher: FileMatcher, test_input: str, expected: MatchedData
) -> None:
...
def test_invalid_year_handling(file_matcher: FileMatcher) -> None:
    ...
    result = file_matcher.match_file_path(
        f"/movies/Future Movie ({future_year}).mkv", MediaType.MOVIE
    )
    assert result.year is None

    # Test too old year
    result = file_matcher.match_file_path(
        "/movies/Ancient Movie (1799).mkv", MediaType.MOVIE
    )
    assert result.year is None
...
def test_nested_brackets_handling(file_matcher: FileMatcher) -> None:
    ...
    result = file_matcher.match_file_path(
        "/movies/Inception [2010] (2010) {2010}.mkv", MediaType.MOVIE
    )
...
def test_multiple_dots_handling(file_matcher: FileMatcher) -> None:
    ...
    result = file_matcher.match_file_path(
        "/movies/The...Matrix...1999...Extended.mkv", MediaType.MOVIE
    )
...
async def test_correct_type_parameter(file_matcher: FileMatcher) -> None:
    ...
    await file_matcher.execute(
        FileMatcherParams(
            path="test",
            media_type=MediaType.MOVIE,
            file_id="6699163b-43ff-41d1-8ce9-0e25630b99f4",
        )
    )
